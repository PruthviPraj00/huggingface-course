...
# Create the Ollama model interface
ollama_model = LiteLLMModel(
    model_id="ollama/qwen2.5:0.5b",  # This model is a bit weak for agentic behaviours though
    api_base="http://localhost:11434",  # replace with 127.0.0.1:11434 or remote open-ai compatible server if necessary
    num_ctx=8192,  # ollama default is 2048 which will fail horribly. 8192 works for easy tasks, more is better. Check https://huggingface.co/spaces/NyxKrage/LLM-Model-VRAM-Calculator to calculate how much VRAM this will need for the selected model.
)

# Initialize the agent
agent = CodeAgent(tools=[party_planning_retriever], model=ollama_model)
